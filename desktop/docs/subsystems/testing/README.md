...
| Configured Ubuntu system build | Clean Debug and Release builds pass with tests enabled and disabled using Qt 6.10.2 plus system media/native dependencies; the Linux player backend is not implemented |
| Linux test suite | All 22 registered CTests and QML lint pass under WSL, including platform-neutral subtitle state, FFmpeg ASS/SubRip/PGS decode, real system-libass rendering, and Linux-shaped dependency contracts; GPU, native-Wayland, physical audio, VAAPI-device, and HDR claims remain gated |
