...
registered startup, playback, and fullscreen application scenarios, but not
yet a broad media corpus, golden-image suite, or general application-scenario
suite. The accepted testing direction is defined in
...
| Configured Ubuntu system build | Clean Debug and Release builds pass with tests enabled and disabled using Qt 6.10.2 plus system media/native dependencies; the production player has a native-Wayland Vulkan backend |
| Linux test suite | All 24 registered CTests and QML lint pass under WSL, including platform-neutral subtitle/media behavior, system dependency contracts, managed/unmanaged Wayland SDR selection, application-chrome state, and packaged QML; final compositor pixel capture, physical audio, VAAPI import, native GPU, and HDR claims remain gated |
| WSLg production boundary | The installed Release player presents software-decoded video through the Qt-owned Vulkan/QRhi/libplacebo path and completes one bounded fullscreen/restoration scenario; two cursor-state timeouts and WSLg configure/protocol diagnostics keep broader lifecycle/fullscreen acceptance in progress |
...
| Actual application fullscreen | Registered CTest drives native F11, Escape, Space, and redirected background double-click input through the real `PresentationWindow`, covering blocked Escape, pause/resume, normal/fullscreen/normal, and maximized/fullscreen/maximized while proving idle cursor hiding survives fullscreen and requiring another video presentation after every transition |
| Built application startup | The built Player executable also has a registered no-window check for packaged QML and production type registration. The application scenarios prove one native-window, graphics-device, swapchain, playback, fullscreen, and automatic-shutdown path; broader command/error/package scenarios remain missing |
