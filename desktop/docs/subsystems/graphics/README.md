...
It is not yet a video renderer. The visible canvas is a procedural HDR test
pattern generated by the final compositor. FFmpeg, libplacebo, decoded video
surfaces, subtitles, and playback scheduling are not integrated.
...
| Video | Procedural diagnostic pattern in the final shader |
...
RhiPresentationEngine
    ...
    └── HdrCompositor
            ├── procedural HDR pattern
            ├── redirected Qt Quick texture
            └── extended-linear or SDR swapchain output
```
...
| `HdrCompositor` | Owns the final graphics pipeline. It currently creates the test pattern, converts the flattened UI layer into the output luminance convention, and writes the swapchain. |
...
2. Disconnect and destroy `QuickUiLayer`, allowing it to invalidate its scene
   and release QRhi resources.
3. Destroy the QRhi device.

The Quick render target intentionally survives swapchain-only recreation. It is
recreated only when its pixel size changes or the whole device is rebuilt.
...
6. Begin the swapchain frame, retrying an out-of-date swapchain after an
   explicit resize.
7. Update compositor parameters and render one fullscreen triangle.
8. End and present the frame.
9. Request another frame only if the pattern is animated or Qt Quick remains
   dirty.
...
* Device loss tears down Quick, swapchain, compositor, and QRhi resources in
  ownership order.
...
The fragment shader currently generates grayscale, color-spectrum, and stepped
ramps with a user-controlled peak. Its optional tone mapper is a simple
diagnostic curve.

This tone mapper is not accepted as the video color pipeline. Once a real video
surface exists, source interpretation, chroma reconstruction, transfer
decoding, tone mapping, gamut mapping, scaling, and dithering belong to
libplacebo. The final compositor must only combine already described layers and
encode the presentation target.
...
4. Scale UI white into the presentation's SDR-white convention.
5. Alpha-composite it over the diagnostic canvas.
...
## Next video-surface boundary

The next implementation slice moves the diagnostic canvas out of the final
compositor and through the same boundary that libplacebo will later use:

```text
temporary diagnostic producer
    → display-targeted RGBA16F video surface
    → final compositor + Qt Quick
    → swapchain
```

At minimum, a rendered surface needs:

* Pixel format.
* Dimensions and valid image region.
* Primaries and transfer function.
* Range and alpha convention.
* Reference-white and luminance convention.
* Whether tone mapping has already occurred.
* Display and graphics-device generation.
* Source frame identity and timestamp when real video arrives.
* Native texture ownership and synchronization rules.

The concrete type should be designed with the first working producer and
consumer rather than as a speculative universal abstraction.
...
policy has table-driven automated coverage. No real GPU capture, renderer image
test, or recorded runtime validation matrix exists yet.
