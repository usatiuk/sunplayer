...
VulkanLibplaceboVideoTarget::VulkanLibplaceboVideoTarget(
    ...
    m_handoffSemaphore = pl_vulkan_sem_create(
        m_gpu, &semaphoreParameters);
    if (!m_handoffSemaphore) {
        setUnavailableDiagnostics(QStringLiteral(
            "Could not create the Vulkan timeline handoff semaphore"));
    } else {
        setUnavailableDiagnostics(QStringLiteral(
...
VideoTargetUpdate VulkanLibplaceboVideoTarget::ensureTarget(
...
VideoOperationResult VulkanLibplaceboVideoTarget::beginProducerAccess(
        QRhiCommandBuffer &) {
    if (deviceLost())
    ...
    if (!native.object)
        return VideoOperationResult::Unavailable;
    const VideoOperationResult signalResult = signalQrhiCompletion();
...
VideoOperationResult VulkanLibplaceboVideoTarget::endProducerAccess(
        QRhiCommandBuffer &) {
    Q_ASSERT(m_libplaceboAccessActive);
    Q_ASSERT(m_submissionPending);
    ...
    if (!pl_vulkan_hold_ex(m_gpu, &hold)) {
        ...
        qCFatal(
            sunroomLogGraphics,
            "Libplacebo failed the mandatory Vulkan image handoff");
    }
    m_compositionBarrierPending = true;
    m_texture->setNativeLayout(
        static_cast<int>(VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL));
    m_libplaceboAccessActive = false;
    return VideoOperationResult::Ready;
}

VideoOperationResult VulkanLibplaceboVideoTarget::prepareForComposition(
...
VulkanLibplaceboVideoTarget::qrhiRenderTarget() const {
    return nullptr;
}

QRhiRenderPassDescriptor *
VulkanLibplaceboVideoTarget::qrhiRenderPassDescriptor() const {
    return nullptr;
}

pl_tex VulkanLibplaceboVideoTarget::libplaceboRenderTarget() const {
    return m_wrappedTexture;
}

QRhiTexture &
VulkanLibplaceboVideoTarget::textureForComposition() const {
    Q_ASSERT(m_texture);
    return *m_texture;
}

std::uint64_t
VulkanLibplaceboVideoTarget::compositionTextureRevision() const {
    return m_compositionTextureRevision;
}

const VideoTargetInteropDiagnostics &
VulkanLibplaceboVideoTarget::diagnostics() const {
    Q_ASSERT(m_diagnostics.isValid());
    return m_diagnostics;
}

VideoTargetUpdate VulkanLibplaceboVideoTarget::createTarget(
    ...
    m_texture->setName(QByteArrayLiteral(
        "Sunroom libplacebo Vulkan video surface"));
...
VideoTargetUpdate VulkanLibplaceboVideoTarget::resizeTarget(
        const QSize &pixelSize) {
    Q_ASSERT(m_texture);
    Q_ASSERT(!pixelSize.isEmpty());
...
bool VulkanLibplaceboVideoTarget::wrapTexture() {
    ...
    if (!native.object) {
        setUnavailableDiagnostics(QStringLiteral(
            "QRhi did not expose its Vulkan video image"));
        return false;
    }
    ...
    if (!m_wrappedTexture || !m_wrappedTexture->params.renderable) {
        pl_tex_destroy(m_gpu, &m_wrappedTexture);
        setUnavailableDiagnostics(QStringLiteral(
            "Libplacebo could not wrap the QRhi Vulkan RGBA16F image"));
        return false;
    }
    return true;
}
...
VideoOperationResult VulkanLibplaceboVideoTarget::signalQrhiCompletion() {
    ...
    const VkResult result = m_deviceFunctions.vkQueueSubmit(
        m_graphicsQueue, 1, &submit, VK_NULL_HANDLE);
    ...
    qCFatal(
        sunroomLogGraphics,
        "Could not order QRhi completion before libplacebo: Vulkan error %d",
        static_cast<int>(result));
    Q_UNREACHABLE_RETURN(VideoOperationResult::Unavailable);
}

void VulkanLibplaceboVideoTarget::recordCompositionBarrier(
        QRhiCommandBuffer &commandBuffer) {
    Q_ASSERT(m_texture);
    const QRhiTexture::NativeTexture native = m_texture->nativeTexture();
    Q_ASSERT(native.object);

    commandBuffer.beginExternal();
    const auto *const nativeCommandBuffer =
        static_cast<const QRhiVulkanCommandBufferNativeHandles *>(
            commandBuffer.nativeHandles());
    if (!nativeCommandBuffer
            || nativeCommandBuffer->commandBuffer == VK_NULL_HANDLE) {
        qCFatal(
            sunroomLogGraphics,
            "QRhi did not expose its Vulkan command buffer");
    }
    ...
    m_deviceFunctions.vkCmdPipelineBarrier2(
        nativeCommandBuffer->commandBuffer, &dependency);
    commandBuffer.endExternal();
    m_texture->setNativeLayout(
        static_cast<int>(VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL));
}

bool VulkanLibplaceboVideoTarget::deviceLost() const {
...
void VulkanLibplaceboVideoTarget::setDirectDiagnostics() {
    ...
    m_diagnostics.synchronizationMode = QStringLiteral(
        "Shared Vulkan graphics queue · timeline dependency · "
        "QRhi command-buffer barrier");
    Q_ASSERT(m_diagnostics.isValid());
}

void VulkanLibplaceboVideoTarget::setUnavailableDiagnostics(
