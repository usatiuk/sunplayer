...
QByteArray droppedRecordMarker(std::uint64_t count) {
    return QStringLiteral(
        "%1 level=warning category=sunroom.application "
...
QByteArray boundedFatalRecord(
    ...
    const qint64 prefixBudget =
        maximumBytes - suffix.size();
    qsizetype low = 0;
    qsizetype high = record.size();
    while (low < high) {
        const qsizetype midpoint =
            low + (high - low + 1) / 2;
        if (record.left(midpoint).toUtf8().size()
                <= prefixBudget) {
            low = midpoint;
        } else {
            high = midpoint - 1;
...
void forwardToDefaultHandler(
    ...
    const QString formatted =
        ...
        + u'\n';
    const QByteArray local = formatted.toLocal8Bit();
    std::fwrite(
        local.constData(),
        1,
        static_cast<std::size_t>(local.size()),
        stderr);
    std::fflush(stderr);
#ifdef Q_OS_WIN
...
bool isSupportedLocalLogPath(
        const QFileInfo &fileInfo) {
#ifdef Q_OS_WIN
    const QString nativePath =
        QDir::toNativeSeparators(
            fileInfo.absoluteFilePath());
    const bool extendedPath =
    ...
    const qsizetype driveOffset =
        extendedPath ? 4 : 0;
    if (nativePath.size() < driveOffset + 3
            || !nativePath[driveOffset]
                .isLetter()
            || nativePath[driveOffset + 1]
                != u':'
            || nativePath[driveOffset + 2]
                != u'\\') {
        return false;
    }
    ...
    const UINT driveType =
        GetDriveTypeW(volumeRoot.c_str());
...
bool ApplicationLogOptions::isValid() const {
    return maximumFileBytes > 0
        && retainedFileCount > 0
        && maximumQueuedBytes >= 256
        && maximumQueuedRecords > 0;
}

std::unique_ptr<ApplicationLog> ApplicationLog::install(
        const ApplicationLogOptions &options,
        QString *error) {
    if (!options.isValid()) {
        if (error) {
            *error = QStringLiteral(
                "Application logging options are invalid");
        }
        return {};
    }

    auto logging = std::unique_ptr<ApplicationLog>(
        new ApplicationLog(options));
...
bool ApplicationLog::debugEnabled() const {
    return m_options.debugEnabled;
}
...
void ApplicationLog::flush() {
    ...
    const std::uint64_t target =
        m_lastEnqueuedSequence;
    if (target <= m_flushedSequence)
        return;
    m_flushRequestedSequence = std::max(
        m_flushRequestedSequence, target);
    lock.unlock();
    m_queueCondition.notify_one();
    lock.lock();
    m_flushCondition.wait(
        lock,
        [this, target] {
            return m_flushedSequence >= target;
        });
}
...
QString ApplicationLog::defaultLogDirectory() {
    ...
    return QDir(temporary).filePath(
        QStringLiteral("Sunroom/logs"));
}
...
bool ApplicationLog::initialize(QString *error) {
    ...
    if (s_active) {
        if (error) {
            *error = QStringLiteral(
                "Application logging is already installed");
        }
        return false;
    }
    ...
    if (m_options.fileEnabled) {
        ...
        const bool automaticPath = path.isEmpty();
        if (automaticPath) {
            path = QDir(defaultLogDirectory()).filePath(
                automaticFileName());
        }
        ...
        if (!isSupportedLocalLogPath(fileInfo)) {
            if (error) {
                *error = QStringLiteral(
                    "Session logs require a local file path");
            }
            return false;
        }
        if (!QDir().mkpath(fileInfo.absolutePath())) {
            if (error) {
                *error = QStringLiteral(
                    "Could not create log directory %1")
        ...
        std::future<QString> initialization =
            initialized.get_future();
    ...
    if (m_options.debugEnabled) {
        QString rules =
            qEnvironmentVariable("QT_LOGGING_RULES");
...
void ApplicationLog::enqueueMessage(
        QtMsgType type,
        const QMessageLogContext &context,
        const QString &message) {
    if (!m_writer.joinable())
    ...
    const QString formatted =
        formattedRecord(type, context, message);
    QByteArray record =
        type == QtFatalMsg
        ? boundedFatalRecord(
            formatted,
        ...
        : formatted.toUtf8();
    {
        ...
        if (type == QtFatalMsg) {
            while ((!m_pendingWrites.empty())
                ...
                PendingWrite evicted =
                    std::move(
                        m_pendingWrites.front());
...
void ApplicationLog::writerLoop(
        ...
        std::uint64_t flushTarget = 0;
        {
            ...
            m_queueCondition.wait(
                ...
                [this, stopToken] {
                    return stopToken.stop_requested()
                        || !m_pendingWrites.empty()
                        || m_droppedRecords > 0
                        || m_flushRequestedSequence
                            > m_flushedSequence;
                });
            ...
            const std::uint64_t
                nextDroppedSequence =
                    ...
                    ? std::numeric_limits<
                        std::uint64_t>::max()
                    : m_firstDroppedSequence;
            if (m_flushRequestedSequence
                        > m_flushedSequence
                    && nextRecordSequence
                        > m_flushRequestedSequence
                    && nextDroppedSequence
                ...
                flushTarget =
                    m_flushRequestedSequence;
            } else if (!m_pendingWrites.empty()
                    && nextRecordSequence
                        <= nextDroppedSequence) {
                action = Action::Record;
                pending =
                    std::move(m_pendingWrites.front());
                m_pendingWrites.pop_front();
...
void ApplicationLog::writeRecord(
        QFile &file,
        const QByteArray &record) {
    const qint64 remaining =
        m_options.maximumFileBytes - m_writtenBytes;
    ...
    if (!m_truncationRecorded) {
        ...
        const QByteArray marker =
            QByteArrayLiteral(
                "level=warning category=sunroom.application "
                "event=log.truncated reason=maximum_file_size\n");
        if (marker.size() <= remaining) {
            const qint64 written = file.write(marker);
            if (written > 0)
                m_writtenBytes += written;
        }
    }
...
void ApplicationLog::writeDroppedRecordMarker(
        QFile &file,
        std::uint64_t count) {
    writeRecord(file, droppedRecordMarker(count));
}

void ApplicationLog::recordDropLocked(
        std::uint64_t sequence) {
    if (m_droppedRecords == 0) {
        m_firstDroppedSequence = sequence;
    } else {
        m_firstDroppedSequence = std::min(
            m_firstDroppedSequence, sequence);
    }
    ++m_droppedRecords;
}
...
void ApplicationLog::messageHandler(
        QtMsgType type,
        const QMessageLogContext &context,
        const QString &message) {
