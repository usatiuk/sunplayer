...
constexpr qsizetype maximumRasterBytes =
    256 * 1024 * 1024;

const QByteArray defaultAssHeader = QByteArrayLiteral(
    "[Script Info]\n"
    "ScriptType: v4.00+\n"
    "PlayResX: 1920\n"
    "PlayResY: 1080\n"
...
void blendAssImage(QImage &target, const ASS_Image &image, QPoint offset) {
    ...
    const unsigned char red = static_cast<unsigned char>(
        (image.color >> 24U) & 0xffU);
    const unsigned char green = static_cast<unsigned char>(
        (image.color >> 16U) & 0xffU);
    const unsigned char blue = static_cast<unsigned char>(
        (image.color >> 8U) & 0xffU);
    const unsigned int opacity = 255U - (image.color & 0xffU);
    for (int y = 0; y < image.h; ++y) {
        auto *output = layer.scanLine(y);
        const unsigned char *coverage = image.bitmap
            + static_cast<std::ptrdiff_t>(y) * image.stride;
        for (int x = 0; x < image.w; ++x) {
            const unsigned int alpha =
                static_cast<unsigned int>(coverage[x]) * opacity / 255U;
    ...
    painter.drawImage(
        offset + QPoint(image.dst_x, image.dst_y), layer);
}

void drawBitmapComposition(
        QImage &target,
        const SubtitleBitmapComposition &composition,
        const QRect &videoRect) {
    if (!composition.isValid() || videoRect.isEmpty())
        return;
    const qreal scaleX = static_cast<qreal>(videoRect.width())
        / composition.canvasSize.width();
    const qreal scaleY = static_cast<qreal>(videoRect.height())
        / composition.canvasSize.height();
    QPainter painter(&target);
    painter.setRenderHint(QPainter::SmoothPixmapTransform, true);
    for (const SubtitleBitmapRegion &region : composition.regions) {
        QImage source(
            reinterpret_cast<const uchar *>(region.rgba.constData()),
            region.size.width(),
            region.size.height(),
            region.size.width() * 4,
            QImage::Format_RGBA8888);
        const QRectF destination(
            videoRect.x() + region.x * scaleX,
            videoRect.y() + region.y * scaleY,
            region.size.width() * scaleX,
            region.size.height() * scaleY);
...
bool SubtitleRenderer::prepare(
        const SubtitlePresentationSnapshot &snapshot,
        const QRect &videoRect,
        const QSize &targetSize,
        bool active) {
    if (!active || !snapshot.state.isEnabled()
            || targetSize.isEmpty() || videoRect.isEmpty()) {
        const bool wasVisible = m_contentVisible;
    ...
    if (m_failedGeneration == m_generation)
        return false;
    if (targetSize.width() > maximumRasterDimension
            || targetSize.height() > maximumRasterDimension
            || static_cast<qint64>(targetSize.width())
                    * targetSize.height() * 4
                > maximumRasterBytes) {
        fail(QStringLiteral("Subtitle raster exceeds its size budget"));
        return false;
    }
    const bool forceRaster = m_textureSize != targetSize
        || m_renderedVideoRect != videoRect;
    if (!processNewEvents(snapshot.state)
            || !ensureTexture(targetSize)) {
        return false;
    }
    m_error.clear();

    const bool requiresRaster =
        forceRaster
        || m_sourceRevision != snapshot.state.revision
        || m_renderedTimeMicroseconds
            != snapshot.mediaTimeMicroseconds
        || m_renderedVideoRect != videoRect
        || m_image.size() != targetSize;
    if (!requiresRaster)
        return true;
...
void SubtitleRenderer::uploadIfNeeded(
...
QRhiTexture *SubtitleRenderer::texture() const {
    return m_texture.get();
}

std::uint64_t SubtitleRenderer::textureRevision() const {
    return m_textureRevision;
}

QString SubtitleRenderer::error() const {
    return m_error;
}

bool SubtitleRenderer::ensureTexture(const QSize &size) {
    ...
    std::unique_ptr<QRhiTexture> texture(
        m_rhi.newTexture(QRhiTexture::RGBA8, size, 1));
    if (!texture) {
...
bool SubtitleRenderer::ensureAssBundle(
    ...
    for (const SubtitleFontAttachment &font : configuration.fonts) {
        ass_add_font(
            bundle->library,
            font.name.toUtf8().constData(),
            font.bytes.constData(),
            font.bytes.size());
    }
    ...
    ass_set_fonts(
        ...
        nullptr,
        1);
    const QByteArray &header = configuration.codecPrivate.isEmpty()
        ? defaultAssHeader
        : configuration.codecPrivate;
    ass_process_codec_private(
        bundle->track,
        const_cast<char *>(header.constData()),
        header.size());
    m_ass = std::move(bundle);
    return true;
}

bool SubtitleRenderer::processNewEvents(
    ...
    for (; m_processedEventCount < state.events->size();
            ++m_processedEventCount) {
        const SubtitleEvent &event =
            (*state.events)[m_processedEventCount];
        if (event.type != SubtitlePayloadType::AssText)
            continue;
        if (!m_ass
                && (!state.configuration
                    || !ensureAssBundle(*state.configuration))) {
            return false;
        }
        const std::int64_t start = event.startMicroseconds / 1'000;
        const std::int64_t duration = event.endMicroseconds
            ? std::max<std::int64_t>(
                1, (*event.endMicroseconds
                    - event.startMicroseconds) / 1'000)
            : 24LL * 60LL * 60LL * 1'000LL;
        ass_process_chunk(
            m_ass->track,
            const_cast<char *>(event.ass.constData()),
            event.ass.size(),
            start,
            duration);
    }
...
bool SubtitleRenderer::rasterize(
        const SubtitlePresentationSnapshot &snapshot,
        const QRect &videoRect,
        const QSize &targetSize,
        bool forceRaster) {
    if (m_image.size() != targetSize) {
        m_image = QImage(
            targetSize, QImage::Format_RGBA8888_Premultiplied);
        if (m_image.isNull()) {
            fail(QStringLiteral("Could not allocate the subtitle raster"));
            return false;
        }
    ...
    for (const SubtitleEvent &event : *snapshot.state.events) {
    ...
    if (m_ass) {
        ass_set_frame_size(
            ...
            videoRect.width(),
            videoRect.height());
        const QSize storage = snapshot.state.configuration->canvasSize.isEmpty()
            ? videoRect.size()
            : snapshot.state.configuration->canvasSize;
        ass_set_storage_size(
            m_ass->renderer, storage.width(), storage.height());
        assImage = ass_render_frame(
            m_ass->renderer,
            m_ass->track,
            snapshot.mediaTimeMicroseconds / 1'000,
            &assChanged);
    }
...
void SubtitleRenderer::clearImage() {
    if (m_image.isNull()) {
        const QSize size = m_texture && !m_textureSize.isEmpty()
            ? m_textureSize
            : QSize(1, 1);
