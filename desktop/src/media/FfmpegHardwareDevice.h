...
public:
    static std::shared_ptr<const FfmpegHardwareDevice> adopt(
    ...
    FfmpegHardwareDevice(const FfmpegHardwareDevice &) = delete;
    FfmpegHardwareDevice &operator=(
        const FfmpegHardwareDevice &) = delete;

    AVBufferRef *referenceDeviceContext() const;
    std::uint64_t graphicsDeviceGeneration() const;
    const QString &apiName() const;
...
struct VideoHardwareDecodeCapability {
    std::shared_ptr<const FfmpegHardwareDevice> device;
