...
QrhiVideoTarget::QrhiVideoTarget(QRhi& rhi, VideoTargetReadback readback) : m_rhi(rhi), m_readback(readback) {
    m_diagnostics.synchronizationMode = QStringLiteral("Not active");
    m_diagnostics.fallbackReason = QStringLiteral("Video target not provisioned");
    Q_ASSERT(m_diagnostics.isValid());
}

QrhiVideoTarget::~QrhiVideoTarget() = default;

VideoTargetUpdate QrhiVideoTarget::ensureTarget(RenderedVideoSurfaceDescription const& description) {
...
VideoOperationResult QrhiVideoTarget::beginProducerAccess(QRhiCommandBuffer&) {
    ...
    Q_ASSERT(!m_submissionPending);
    Q_ASSERT(!m_compositionPrepared);
    if (!m_texture || m_producerAccessActive || m_submissionPending || m_compositionPrepared) {
...
VideoOperationResult QrhiVideoTarget::endProducerAccess(QRhiCommandBuffer&) {
    if (m_rhi.isDeviceLost()) {
        return VideoOperationResult::DeviceLost;
    }
    Q_ASSERT(m_producerAccessActive);
...
VideoOperationResult QrhiVideoTarget::prepareForComposition(QRhiCommandBuffer&) {
    if (m_rhi.isDeviceLost()) {
        return VideoOperationResult::DeviceLost;
    }
    Q_ASSERT(!m_producerAccessActive);
    Q_ASSERT(!m_compositionPrepared);
    if (!m_texture || m_producerAccessActive || m_compositionPrepared) {
        return VideoOperationResult::Unavailable;
    }
    m_submissionPending = true;
...
QRhiTextureRenderTarget* QrhiVideoTarget::qrhiRenderTarget() const { return m_renderTarget.get(); }

QRhiRenderPassDescriptor* QrhiVideoTarget::qrhiRenderPassDescriptor() const { return m_renderPassDescriptor.get(); }

pl_tex QrhiVideoTarget::libplaceboRenderTarget() const { return nullptr; }
...
std::uint64_t QrhiVideoTarget::compositionTextureRevision() const { return m_compositionTextureRevision; }

VideoTargetInteropDiagnostics const& QrhiVideoTarget::diagnostics() const {
    Q_ASSERT(m_diagnostics.isValid());
    return m_diagnostics;
}

VideoTargetUpdate QrhiVideoTarget::createTarget(QSize const& pixelSize) {
...
VideoTargetUpdate QrhiVideoTarget::resizeTarget(QSize const& pixelSize) {
    Q_ASSERT(m_texture);
    Q_ASSERT(!pixelSize.isEmpty());
...
void QrhiVideoTarget::setDirectDiagnostics() {
    ...
    m_diagnostics.synchronizationMode = QStringLiteral("QRhi command-buffer ordering");
    Q_ASSERT(m_diagnostics.isValid());
}

void QrhiVideoTarget::setUnavailableDiagnostics(QString const& reason) {
    m_diagnostics = {};
