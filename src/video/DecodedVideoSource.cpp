...
std::uint64_t
DecodedVideoSource::producerConfigurationRevision() const {
    return m_producerConfigurationRevision;
}

std::optional<double>
DecodedVideoSource::displayAspectRatio() const {
    Q_ASSERT(QThread::currentThread() == thread());
    if (!m_frame)
        return std::nullopt;
    ...
    const VideoFrameGeometry &geometry =
        m_frame->geometry();
    double width =
        static_cast<double>(geometry.visibleSize.width());
    const double height =
        static_cast<double>(geometry.visibleSize.height());
    if (geometry.sampleAspectRatioKnown) {
        width *= static_cast<double>(
            geometry.sampleAspectRatio.numerator)
            / static_cast<double>(
                geometry.sampleAspectRatio.denominator);
    }
    ...
    double rotation = std::fmod(
        std::abs(geometry.rotationDegrees), 180.0);
