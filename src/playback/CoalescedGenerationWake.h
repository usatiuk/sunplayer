...
class CoalescedGenerationWake final {
  public:
    ...
    template <typename Handler> void drain(Handler&& handler) {
        while (true) {
            std::uint64_t const generation = m_latestGeneration.exchange(0);
            if (generation != 0) {
                handler(generation);
            }

            m_pending.store(false);
            if (m_latestGeneration.load() == 0 || m_pending.exchange(true)) {
                return;
            }
        }
    }
