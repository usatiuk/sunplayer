...
class VideoFrameTimeline final {
  public:
    explicit VideoFrameTimeline(std::optional<VideoTimelineOrigin> stableOrigin = {});

    QueuedVideoFrame schedule(std::shared_ptr<DecodedVideoFrame const> frame,
...
class VideoSeekPrerollGate final {
  public:
    explicit VideoSeekPrerollGate(std::optional<std::int64_t> targetPositionMicroseconds);

    VideoSeekPrerollAdmission admit(QueuedVideoFrame frame);
...
class VideoFrameQueue final {
  public:
    ...
    bool push(std::uint64_t playbackGeneration, QueuedVideoFrame frame, std::stop_token stopToken);
    std::optional<QueuedVideoFrame> front(std::uint64_t playbackGeneration) const;
    std::optional<QueuedVideoFrame> pop(std::uint64_t playbackGeneration);
    std::size_t size(std::uint64_t playbackGeneration) const;
    std::uint64_t totalPushed(std::uint64_t playbackGeneration) const;
