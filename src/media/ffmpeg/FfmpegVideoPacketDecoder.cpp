...
enum AVPixelFormat selectHardwareFormat(AVCodecContext* context, const enum AVPixelFormat* formats) {
    ...
    if (state.fallbackReason.isEmpty()) {
        state.fallbackReason = QStringLiteral("The decoder did not offer the requested %1 format").arg(state.apiName);
    }
    return avcodec_default_get_format(context, formats);
}

AVCodecHWConfig const* hardwareConfiguration(AVCodec const& decoder, enum AVHWDeviceType deviceType) {
    for (int index = 0;; ++index) {
        AVCodecHWConfig const* configuration = avcodec_get_hw_config(&decoder, index);
        if (!configuration) {
            return nullptr;
        }
        if (configuration->device_type == deviceType && configuration->pix_fmt != AV_PIX_FMT_NONE &&
...
QByteArray streamHdr10PlusMetadata(AVCodecParameters const& parameters) {
    if (AVPacketSideData const* hdr10Plus = av_packet_side_data_get(
            parameters.coded_side_data, parameters.nb_coded_side_data, AV_PKT_DATA_DYNAMIC_HDR10_PLUS)) {
        if (hdr10Plus->size > 0 && hdr10Plus->size <= static_cast<std::size_t>(std::numeric_limits<qsizetype>::max())) {
            return QByteArray(reinterpret_cast<char const*>(hdr10Plus->data), static_cast<qsizetype>(hdr10Plus->size));
        }
    }
    return {};
}
...
bool attachStreamHdr10PlusMetadata(AVFrame& frame, QByteArray const& metadata) {
    if (metadata.isEmpty() || av_frame_get_side_data(&frame, AV_FRAME_DATA_DYNAMIC_HDR_PLUS)) {
        return true;
    }
    AVFrameSideData* sideData =
        av_frame_new_side_data(&frame, AV_FRAME_DATA_DYNAMIC_HDR_PLUS, static_cast<std::size_t>(metadata.size()));
    if (!sideData) {
        return false;
    }
    std::memcpy(sideData->data, metadata.constData(), static_cast<std::size_t>(metadata.size()));
    return true;
}
...
void FfmpegAvPacketDeleter::operator()(AVPacket* packet) const { av_packet_free(&packet); }
...
decodeFfmpegVideoPackets(FfmpegVideoDecodeRequest const& request, AVCodec const& decoder,
    ...
    *hardwareSelected = false;
    ...
    auto const endOfStream = [&result] {
        if (result.framesDecoded == 0) {
            result.error = QStringLiteral("The selected video stream produced no decoded frame");
        } else {
            result.endOfStream = true;
            std::optional<std::int64_t> const duration =
                observedPlaybackDurationMicroseconds(result.observedEndMicroseconds, std::nullopt);
            if (duration) {
                result.diagnostics.durationMicroseconds = duration;
    ...
    hardwareState.fallbackReason = request.hardwareDecode.unavailableReason;

    CodecContextPtr codecContext(avcodec_alloc_context3(&decoder));
    if (!codecContext) {
        return fail(QStringLiteral("Could not allocate the video decoder"));
    }
    int status = avcodec_parameters_to_context(codecContext.get(), &streamParameters);
    if (status < 0) {
        return fail(QStringLiteral("Could not configure the video decoder: %1").arg(ffmpegError(status)));
    }
    ...
    if (request.hardwareDecode.device) {
        AVBufferRef* deviceReference = request.hardwareDecode.device->referenceDeviceContext();
        if (!deviceReference) {
            hardwareState.fallbackReason =
                QStringLiteral("Could not retain the %1 device").arg(request.hardwareDecode.device->apiName());
        } else {
            auto const* const deviceContext = reinterpret_cast<AVHWDeviceContext const*>(deviceReference->data);
            AVCodecHWConfig const* configuration =
                deviceContext ? hardwareConfiguration(decoder, deviceContext->type) : nullptr;
            if (!configuration) {
                hardwareState.fallbackReason =
                    QStringLiteral("Decoder %1 does not support %2")
                        .arg(QString::fromLatin1(decoder.name), request.hardwareDecode.device->apiName());
                av_buffer_unref(&deviceReference);
            } else {
                hardwareState.pixelFormat = configuration->pix_fmt;
                hardwareState.graphicsDeviceGeneration = request.hardwareDecode.device->graphicsDeviceGeneration();
                hardwareState.apiName = request.hardwareDecode.device->apiName();
                codecContext->opaque = &hardwareState;
                codecContext->get_format = selectHardwareFormat;
                codecContext->hw_device_ctx = deviceReference;
                codecContext->extra_hw_frames = request.extraHardwareFrames;
            }
        }
    }
    ...
    if (status < 0) {
        ...
        return fail(QStringLiteral("Could not open video decoder %1: %2")
                        .arg(QString::fromLatin1(decoder.name), ffmpegError(status)));
    }

    FramePtr frame(av_frame_alloc());
    if (!frame) {
        return fail(QStringLiteral("Could not allocate FFmpeg frame storage"));
    }

    FfmpegVideoStreamDiagnostics softwareDiagnostics = baseDiagnostics;
    softwareDiagnostics.hardwareAccelerated = false;
    softwareDiagnostics.decodePath = QStringLiteral("Software");
    softwareDiagnostics.hardwareFallbackReason = hardwareState.fallbackReason;
    FfmpegVideoStreamDiagnostics hardwareDiagnostics = baseDiagnostics;
    ...
    std::optional<VideoTimelineOrigin> resolvedTimelineOrigin =
        request.start.timelineOrigin ? request.start.timelineOrigin : baseDiagnostics.timelineOrigin;
    ...
    auto const drainDecoder = [&](bool flushing) -> DrainResult {
        while (true) {
            ...
            int const receiveStatus = avcodec_receive_frame(codecContext.get(), frame.get());
            if (receiveStatus == AVERROR(EAGAIN)) {
                if (flushing) {
                    result.error = QStringLiteral("Video decoder requested input after "
            ...
            if (receiveStatus < 0) {
                ...
                result.error = QStringLiteral("Video decoding failed: %1").arg(ffmpegError(receiveStatus));
                return DrainResult::Failed;
            }
            ...
            if (nextFrameId == 0) {
                ...
                result.error = QStringLiteral("Decoded frame identity overflowed");
                return DrainResult::Failed;
            }
            VideoFrameIdentity const identity{
                .playbackGeneration = request.firstFrameIdentity.playbackGeneration,
                .decoderRevision = request.firstFrameIdentity.decoderRevision,
                .frameId = nextFrameId++,
            };
            QString frameError;
            bool const hardwareFrame = frame->format == hardwareState.pixelFormat && frame->hw_frames_ctx;
            if (!attachStreamHdr10PlusMetadata(*frame, hdr10PlusMetadata)) {
                result.error = QStringLiteral("Could not attach stream-level HDR10+ metadata");
                return DrainResult::Failed;
            }
            ...
            FfmpegVideoStreamDiagnostics diagnostics = hardwareFrame ? hardwareDiagnostics : softwareDiagnostics;
            if (!resolvedTimelineOrigin && decoded->timing().pts) {
            ...
            if (!hardwareFrame) {
                diagnostics.hardwareFallbackReason = hardwareState.fallbackReason;
            }
            if (!result.diagnostics.isValid()) {
                result.diagnostics = diagnostics;
            }
            std::optional<std::int64_t> const pts = decoded->timing().ptsMicroseconds();
            std::optional<std::int64_t> const duration = decoded->timing().durationMicroseconds();
            std::optional<std::int64_t> const origin =
                resolvedTimelineOrigin ? resolvedTimelineOrigin->microseconds() : std::nullopt;
            observedVideoRange.observeFrame(pts && origin ? checkedTimestampSubtract(*pts, *origin) : std::nullopt,
                                            duration);
            result.observedEndMicroseconds = observedVideoRange.endMicroseconds();
            ...
            if (result.framesDecoded == 1 ||
                ...
                qCDebug(sunplayerLogMediaDecode).noquote()
                    ...
                    << "pts=" +
                           (decoded->timing().pts ? QString::number(*decoded->timing().pts) : QStringLiteral("none"))
                    << "ptsUs=" + (decoded->timing().ptsMicroseconds()
                                       ? QString::number(*decoded->timing().ptsMicroseconds())
                                       : QStringLiteral("none"))
                    << "hardware=" + QString(hardwareFrame ? QStringLiteral("true") : QStringLiteral("false"))
                    << "elapsedMs=" + QString::number(operationTimer.elapsed());
            }
            if (!sink(std::move(decoded), diagnostics)) {
                return stopToken.stop_requested() ? DrainResult::Cancelled : DrainResult::Stopped;
            }
        }
    };
    ...
    while (!stopToken.stop_requested()) {
        ...
        if (stopToken.stop_requested() || input.terminal == FfmpegVideoPacketTerminal::Cancelled) {
            return cancel();
        }
        if (input.terminal == FfmpegVideoPacketTerminal::Failed) {
            return fail(input.error.isEmpty() ? QStringLiteral("Media demuxing failed") : input.error);
        }
        if (input.terminal == FfmpegVideoPacketTerminal::EndOfStream) {
            while (true) {
                status = avcodec_send_packet(codecContext.get(), nullptr);
                if (status != AVERROR(EAGAIN)) {
                    break;
                }
                DrainResult const progress = drainDecoder(false);
                ...
                if (progress == DrainResult::NeedInput) {
                    return fail(QStringLiteral("Video decoder made no progress "
            ...
            if (status < 0 && status != AVERROR_EOF) {
                return fail(QStringLiteral("Could not flush video decoder: %1").arg(ffmpegError(status)));
            }
            if (status == AVERROR_EOF) {
                return endOfStream();
            }
            DrainResult const drained = drainDecoder(true);
            ...
            if (drained != DrainResult::Drained) {
                return fail(QStringLiteral("Video decoder did not reach end of stream"));
            }
            return endOfStream();
        }

        if (!input.packet) {
            return fail(QStringLiteral("The video packet source returned no packet"));
        }
        while (true) {
            status = avcodec_send_packet(codecContext.get(), input.packet.get());
            if (status != AVERROR(EAGAIN)) {
                break;
            }
            std::uint64_t const before = result.framesDecoded;
            DrainResult const progress = drainDecoder(false);
            if (progress == DrainResult::Stopped) {
                result.stopped = true;
                return result;
            ...
            if (progress != DrainResult::Drained && result.framesDecoded == before) {
                return fail(QStringLiteral("Video decoder returned EAGAIN from both "
        ...
        if (status < 0) {
            ...
            return fail(QStringLiteral("Could not submit video packet: %1").arg(ffmpegError(status)));
        }

        DrainResult const drained = drainDecoder(false);
        if (drained == DrainResult::Stopped) {
            result.stopped = true;
            return result;
        }
