...
namespace {
bool validRgbaSize(QSize const& size, qsizetype byteCount) {
    if (size.isEmpty() || size.width() <= 0 || size.height() <= 0) {
        return false;
    }
    constexpr qsizetype channels = 4;
    if (size.width() > std::numeric_limits<qsizetype>::max() / channels) {
        return false;
    }
    qsizetype const rowBytes = size.width() * channels;
    if (size.height() > std::numeric_limits<qsizetype>::max() / rowBytes) {
        return false;
    }
...
bool SubtitleBitmapRegion::isValid() const { return x >= 0 && y >= 0 && validRgbaSize(size, rgba.size()); }

bool SubtitleBitmapComposition::isValid() const {
    if (canvasSize.isEmpty() || regions.empty()) {
        return false;
    }
    for (SubtitleBitmapRegion const& region : regions) {
        if (!region.isValid() || region.x > canvasSize.width() || region.size.width() > canvasSize.width() - region.x ||
            region.y > canvasSize.height() || region.size.height() > canvasSize.height() - region.y) {
            return false;
        }
    }
    return true;
...
bool SubtitleStateSnapshot::isEnabled() const {
    return playbackGeneration != 0 && configuration && configuration->isValid() && events && error.isEmpty();
}
