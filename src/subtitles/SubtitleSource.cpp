...
constexpr std::size_t maximumRetainedPayloadBytes = 256U * 1024U * 1024U;

std::optional<std::size_t> retainedPayloadBytes(SubtitleEvent const& event) {
    std::size_t bytes = static_cast<std::size_t>(event.ass.size());
    if (!event.bitmap) {
        return bytes;
    }
    for (SubtitleBitmapRegion const& region : event.bitmap->regions) {
        std::size_t const regionBytes = static_cast<std::size_t>(region.rgba.size());
...
void SubtitleSource::configure(SubtitleStreamConfiguration configuration) {
    if (!configuration.isValid()) {
        return;
    }
    std::lock_guard lock(m_mutex);
    if (configuration.playbackGeneration != m_playbackGeneration) {
        return;
    }
    m_configuration = std::make_shared<SubtitleStreamConfiguration const>(std::move(configuration));
    m_events = std::make_shared<std::vector<SubtitleEvent>>();
    m_retainedPayloadBytes = 0;
    m_error.clear();
    advanceRevision();
...
bool SubtitleSource::append(SubtitleEvent event) {
    ...
    std::lock_guard lock(m_mutex);
    if (event.playbackGeneration != m_playbackGeneration || !m_configuration || !m_error.isEmpty()) {
        return false;
    }
    std::optional<std::size_t> const eventBytes = retainedPayloadBytes(event);
    if (!eventBytes || m_events->size() >= maximumRetainedEvents ||
        *eventBytes > maximumRetainedPayloadBytes - std::min(m_retainedPayloadBytes, maximumRetainedPayloadBytes)) {
...
void SubtitleSource::fail(std::uint64_t playbackGeneration, QString error) {
    if (error.isEmpty()) {
        return;
    }
    std::lock_guard lock(m_mutex);
