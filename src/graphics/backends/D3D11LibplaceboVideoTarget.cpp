...
D3D11LibplaceboVideoTarget::D3D11LibplaceboVideoTarget(
        QRhi &rhi,
        pl_gpu gpu,
    ...
    setUnavailableDiagnostics(
        QStringLiteral("Libplacebo video target not provisioned"));
}
...
VideoTargetUpdate D3D11LibplaceboVideoTarget::ensureTarget(
...
D3D11LibplaceboVideoTarget::beginProducerAccess(
    ...
    Q_ASSERT(!m_submissionPending);
    Q_ASSERT(!m_compositionPrepared);
    if (!m_wrappedTexture || !m_renderTarget
            || m_producerAccessActive || m_submissionPending
            || m_compositionPrepared) {
...
VideoOperationResult
D3D11LibplaceboVideoTarget::endProducerAccess(
        QRhiCommandBuffer &commandBuffer) {
    Q_ASSERT(m_producerAccessActive);
    Q_ASSERT(m_submissionPending);
    if (!m_producerAccessActive || !m_submissionPending)
...
VideoOperationResult
D3D11LibplaceboVideoTarget::prepareForComposition(
        QRhiCommandBuffer &) {
    if (deviceLost())
        return VideoOperationResult::DeviceLost;
    Q_ASSERT(!m_producerAccessActive);
...
D3D11LibplaceboVideoTarget::qrhiRenderTarget() const {
    return m_renderTarget.get();
}

QRhiRenderPassDescriptor *
D3D11LibplaceboVideoTarget::qrhiRenderPassDescriptor() const {
    return m_renderPassDescriptor.get();
}

pl_tex D3D11LibplaceboVideoTarget::libplaceboRenderTarget() const {
    return m_wrappedTexture;
}

QRhiTexture &
D3D11LibplaceboVideoTarget::textureForComposition() const {
    Q_ASSERT(m_texture);
    return *m_texture;
}

std::uint64_t
D3D11LibplaceboVideoTarget::compositionTextureRevision() const {
    return m_compositionTextureRevision;
}

const VideoTargetInteropDiagnostics &
D3D11LibplaceboVideoTarget::diagnostics() const {
    Q_ASSERT(m_diagnostics.isValid());
    return m_diagnostics;
}

VideoTargetUpdate D3D11LibplaceboVideoTarget::createTarget(
    ...
    m_texture->setName(
        QByteArrayLiteral("Sunroom libplacebo video surface"));
    if (!m_texture->create()) {
        const bool lost = deviceLost();
        setUnavailableDiagnostics(lost
            ? QStringLiteral("Graphics device lost")
            : QStringLiteral(
                "Could not create the libplacebo video texture"));
        resetTarget();
        return lost
            ? VideoTargetUpdate::DeviceLost
            : VideoTargetUpdate::Unavailable;
    ...
    m_renderTarget.reset(
        m_rhi.newTextureRenderTarget(description));
    m_renderTarget->setName(QByteArrayLiteral(
        "Sunroom libplacebo external render target"));
    ...
    if (!m_renderTarget->create()) {
        ...
        setUnavailableDiagnostics(lost
            ? QStringLiteral("Graphics device lost")
            : QStringLiteral(
                "Could not create the external-command render target"));
        resetTarget();
        return lost
            ? VideoTargetUpdate::DeviceLost
            : VideoTargetUpdate::Unavailable;
...
VideoTargetUpdate D3D11LibplaceboVideoTarget::resizeTarget(
        const QSize &pixelSize) {
    Q_ASSERT(m_texture);
    Q_ASSERT(!pixelSize.isEmpty());
    ...
    if (!m_texture->create()) {
        ...
        setUnavailableDiagnostics(lost
            ? QStringLiteral("Graphics device lost")
            : QStringLiteral(
                "Could not resize the libplacebo video texture"));
        resetTarget();
        return lost
            ? VideoTargetUpdate::DeviceLost
            : VideoTargetUpdate::Unavailable;
...
bool D3D11LibplaceboVideoTarget::wrapTexture() {
    ...
    auto *const texture = reinterpret_cast<ID3D11Texture2D *>(
        static_cast<std::uintptr_t>(native.object));
    if (!texture) {
        setUnavailableDiagnostics(QStringLiteral(
            "QRhi did not expose its D3D11 video texture"));
        return false;
    }
    ...
    if (description.Format != DXGI_FORMAT_R16G16B16A16_FLOAT
        ...
        setUnavailableDiagnostics(QStringLiteral(
            "QRhi exposed an incompatible D3D11 video texture"));
        return false;
    }
    ...
    if (!m_wrappedTexture || !m_wrappedTexture->params.renderable) {
        pl_tex_destroy(m_gpu, &m_wrappedTexture);
        setUnavailableDiagnostics(QStringLiteral(
            "Libplacebo could not wrap the QRhi RGBA16F render target"));
        return false;
    }
    return true;
}

bool D3D11LibplaceboVideoTarget::deviceLost() const {
...
void D3D11LibplaceboVideoTarget::setDirectDiagnostics() {
    ...
    m_diagnostics.synchronizationMode = QStringLiteral(
        "Shared D3D11 immediate context · QRhi external commands · %1")
        .arg(QString::fromLatin1(
            m_wrappedTexture->params.format->name));
    Q_ASSERT(m_diagnostics.isValid());
}

void D3D11LibplaceboVideoTarget::setUnavailableDiagnostics(
