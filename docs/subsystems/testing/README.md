...
| Microsoft Store MSIX boundary | On Windows 11 build 26200, a clean Release configure/build/install and direct QML probe pass. Pinned `winapp` loose identity passes packaged-QML and the real fullscreen/playback/default-audio scenario, then unregisters. A signed development 1.0.0.0 package verifies through SignTool with zero warnings/errors, installs, and cold-launches; a stable-identity 1.0.1.0 update installs and cold-launches; uninstall and exact temporary-certificate cleanup leave no package, process, or trust entry. Unsigned development packaging unpacks and validates the x64 manifest, CMake-owned runtime closure, qualified-asset PRI data, absent signature, exclusions, hashes, and inventory. The source-controlled Store identity/version path, clean-machine and LTSC-2019 floor, packaged settings mutation/relaunch, and Store-delivered flight/update evidence remain open |
| Configured Windows Debug build | Builds successfully; the 2026-08-09 outside-sandbox MSVC build completed with 14 parallel jobs using the existing CLion-configured tree |
