...
| Configured macOS development build | Apple M2/macOS 26 Debug and clean Release source builds succeed with Qt 6.11.1, stock vcpkg `arm64-osx`, QRhi Metal, libplacebo Vulkan/MoltenVK, VideoToolbox, AudioUnit, and libass; the final Debug suite passes 26/26 without a leaked application process; macOS 13 dependency compatibility and bundle closure remain packaging gates |
| Focused automated tests | Windows registers twenty-nine CTest targets, including the native pre-presentation background probe; the suite also covers presentation policy, UI/session lifecycle, bounded media/audio/subtitle queues and state, timing and generation behavior, dependency boundaries, real FFmpeg A/V/subtitle decode, libswresample/libass rendering, a silent real-WASAPI sink lifecycle, real GPU paths, and bounded application playback/fullscreen scenarios |
...
| Linux test suite | The current Debug build passes all 26 registered CTests together and QML lint under WSL, including system cubeb, application playback, shared subtitle/media behavior, dependencies, managed/unmanaged Wayland SDR selection, application chrome, and packaged QML. Current focused coverage also exercises version-2 managed HDR10 capability/stable-mode selection, 1x and lower compositor targets, output-encoding coupling, the PQ coordinate cap, and graphics-generation rejection. Native route change/acoustics, VAAPI import, native GPU, Wayland HDR10 WSI, and physical HDR claims remain gated |
