...
| Configured Windows Debug build | Builds successfully; on 2026-08-22 the absolute-PQ target construction rebuilt the complete existing CLion-configured tree outside the sandbox with MSVC, and all 31 registered tests passed together. The focused presentation-target, rendered-surface, color-policy, and real FFmpeg/libplacebo boundaries pass together in Debug and RelWithDebInfo; the complete player target also builds cleanly. The earlier `media-session` exact-1-ms timing failure did not recur in either its isolated rerun or the final full-tree runs. |
| Configured macOS development build | Apple M2/macOS 26 Debug and clean Release source builds succeed with Qt 6.11.1, stock vcpkg `arm64-osx`, QRhi Metal, libplacebo Vulkan/MoltenVK, VideoToolbox, AudioUnit, and libass. The prior final Debug suite passed 26/26 without a leaked application process; the current 28-test tree awaits a new macOS run. macOS 13 dependency compatibility and bundle closure remain packaging gates |
| Focused automated tests | Windows registers 31 CTest targets, including the shared color-policy test, focused application-settings coverage, and the native pre-presentation background probe; the suite also covers presentation policy, UI/session lifecycle, bounded media/audio/subtitle queues and state, timing and generation behavior, dependency boundaries, real FFmpeg A/V/subtitle decode, libswresample/libass rendering, a silent real-WASAPI sink lifecycle, real GPU paths, and bounded application playback/fullscreen scenarios |
...
| Linux test suite | The last validated Debug build passed all 26 then-registered CTests together and QML lint under WSL. The current CMake tree registers 28 after adding the platform-neutral application-settings and color-policy tests; a new Linux run is pending. Coverage includes system cubeb, application playback, shared subtitle/media behavior, dependencies, managed/unmanaged Wayland SDR selection, application chrome, packaged QML, version-2 managed HDR10 capability/stable-mode selection, 1x and lower compositor targets, output-encoding coupling, the PQ coordinate cap, and graphics-generation rejection. Native route change/acoustics, VAAPI import, native GPU, Wayland HDR10 WSI, and physical HDR claims remain gated |
...
| Built application startup | The built Player executable has a registered no-window check for packaged QML and production type registration plus a Windows-only real-window native-message probe requiring black client erase before first QRhi presentation. On 2026-08-09 the Windows probe, packaged-QML, QML-shell, playback, and fullscreen scenarios pass together, and a user check confirms the visible white flash is gone in the current Windows desktop mode. The other SDR/HDR desktop mode remains a physical matrix gap, and broader command/error/package scenarios remain missing |
