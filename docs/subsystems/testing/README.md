...
the real render boundary, SDR-target operator selection, and remapping of the
same paused dual-format frame across SDR/HDR target classes.
...
| Configured Windows Debug build | Builds successfully; on 2026-08-21 the full tree rebuilt outside the sandbox with MSVC using the existing CLion-configured tree and all 30 reliable tests passed together after excluding the separately reproduced `media-session` exact-1-ms timing failure. The metadata-first policy and real FFmpeg/libplacebo boundary also pass in RelWithDebInfo. |
