...
| Configured Windows Debug build | Builds successfully; on 2026-08-22 the complete existing CLion-configured tree rebuilt outside the sandbox with MSVC, both QML lint targets passed, and all 34 registered tests passed together. The latest run includes the reference-white HDR correction, real decoded/compositor scaling regression, pinned HDR10+ nominal-SDR pixel lock, pure target calculation, and the existing GPU/hardware-decode cases. It also includes the pure drop policy, QML shared-ingress route, strict positional cardinality, real native-window drop, and advertised FFmpeg demuxer families. A RelWithDebInfo player also rebuilt, installed, passed packaged-QML startup, and produced a schema-valid unsigned MSIX containing the reviewed association. The earlier `media-session` exact-1-ms timing failure did not recur in either its isolated rerun or the final full-tree runs. |
| Configured macOS development build | Apple M2/macOS 26 Debug and clean Release source builds succeed with Qt 6.11.1, stock vcpkg `arm64-osx`, QRhi Metal, libplacebo Vulkan/MoltenVK, VideoToolbox, AudioUnit, and libass. The prior final Debug suite passed 26/26 without a leaked application process; the current 30-test tree awaits a new macOS run. macOS 13 dependency compatibility and bundle closure remain packaging gates |
| Focused automated tests | Windows registers 34 CTest targets, including shared color policy, application settings, single-local-drop policy, positional-cardinality rejection, a native pre-presentation background probe, and a real native-window drop probe; the suite also covers presentation policy, UI/session lifecycle, release-ordered and cancellation-safe background fullscreen gestures, bounded media/audio/subtitle queues and state, timing and generation behavior, dependency boundaries, real FFmpeg A/V/subtitle decode, libswresample/libass rendering, a silent real-WASAPI sink lifecycle, real GPU paths, and bounded application playback/fullscreen scenarios |
...
| Linux test suite | The last validated Debug build passed all 26 then-registered CTests together and QML lint under WSL. The current CMake tree registers 30 after adding the platform-neutral application-settings, color-policy, local-drop-policy, and positional-cardinality tests; a new Linux run is pending. Coverage includes system cubeb, application playback, shared subtitle/media behavior, dependencies, managed/unmanaged Wayland SDR selection, application chrome, packaged QML, version-2 managed HDR10 capability/stable-mode selection, 1x and lower compositor targets, output-encoding coupling, the PQ coordinate cap, and graphics-generation rejection. Native route change/acoustics, VAAPI import, native GPU, Wayland HDR10 WSI, and physical HDR claims remain gated |
...
| Single-file application ingress | Pure policy covers one local or UNC URL and rejects null, remote, empty, multiple, and move-only drops; QML proves the file dialog uses the shared application command and open requests return HDR Lab to Player; an actual Windows window probe requires copy acceptance and exact session delivery; a process scenario rejects multiple positional media paths. The RelWithDebInfo install and unsigned MSIX package pass, while a signed/loose installed-package Open-with launch remains a manual release check. |
