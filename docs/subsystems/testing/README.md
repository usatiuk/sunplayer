...
| Configured Windows Debug build | Builds successfully; on 2026-08-19 the affected application, QML-shell, presentation-target, rendered-surface, and real-QRhi targets rebuilt outside the sandbox with MSVC using the existing CLion-configured tree, and the five focused CTests passed together |
| Configured macOS development build | Apple M2/macOS 26 Debug and clean Release source builds succeed with Qt 6.11.1, stock vcpkg `arm64-osx`, QRhi Metal, libplacebo Vulkan/MoltenVK, VideoToolbox, AudioUnit, and libass. The prior final Debug suite passed 26/26 without a leaked application process; the current 27-test tree awaits a new macOS run. macOS 13 dependency compatibility and bundle closure remain packaging gates |
| Focused automated tests | Windows registers thirty CTest targets, including focused application-settings coverage and the native pre-presentation background probe; the suite also covers presentation policy, UI/session lifecycle, bounded media/audio/subtitle queues and state, timing and generation behavior, dependency boundaries, real FFmpeg A/V/subtitle decode, libswresample/libass rendering, a silent real-WASAPI sink lifecycle, real GPU paths, and bounded application playback/fullscreen scenarios |
...
| Linux test suite | The last validated Debug build passed all 26 then-registered CTests together and QML lint under WSL. The current CMake tree registers 27 after adding the platform-neutral application-settings test; a new Linux run is pending. Coverage includes system cubeb, application playback, shared subtitle/media behavior, dependencies, managed/unmanaged Wayland SDR selection, application chrome, packaged QML, version-2 managed HDR10 capability/stable-mode selection, 1x and lower compositor targets, output-encoding coupling, the PQ coordinate cap, and graphics-generation rejection. Native route change/acoustics, VAAPI import, native GPU, Wayland HDR10 WSI, and physical HDR claims remain gated |
