...
item and therefore selects SunPlayer's hidden redirected `QQuickWindow`.
...
`FileDialog.parentWindow` to SunPlayer's visible `PresentationWindow` on macOS.
...
parent. SunPlayer's normal macOS path uses the native dialog. Deliberate
...
When SunPlayer updates Qt, verify the shipped patch level and both native and
