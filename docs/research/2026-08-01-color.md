# SunPlayer video-rendering and color-management milestone review
...
* **[Policy]** Recommended SunPlayer decision.
...
SunPlayer should adopt the proposed display-relative working representation:
...
3. `DisplayTargetSnapshot`: SunPlayer’s policy result for libplacebo and final platform presentation.
...
libplacebo documents that some FFmpeg stream-level side data is not propagated to every frame and offers `pl_frame_copy_stream_props` to add it. It does not provide SunPlayer’s required conflict policy, provenance, confidence, or diagnostic explanation. ([GitHub][6])

**[Policy]** Use libplacebo’s helpers to parse and map values, but make SunPlayer’s immutable `EffectiveSourceDescription` the sole policy decision. Never let a late helper call silently redefine already-resolved frame metadata.
...
`DXGI_OUTPUT_DESC1` reports color-volume and luminance information, but Windows does not expose a direct Apple-style continuously varying “current EDR headroom” value. SunPlayer must distinguish:
...
Setting `CAMetalLayer.edrMetadata` opts the layer into Apple’s media tone mapper. If libplacebo has already adapted the video, doing this would introduce a second tone-mapping stage. SunPlayer’s libplacebo-owned path should therefore leave `edrMetadata` unset. ([Apple Developer][9])
...
2. Resolve SunPlayer’s effective source snapshot.
...
| Who performs HDR adaptation | SunPlayer/libplacebo                                                                              | SunPlayer/libplacebo                                                                                | SunPlayer/libplacebo; compositor handles conversion to outputs                                                 | SunPlayer maps HDR to SDR           |
...
Apple’s EDR model directly matches SunPlayer’s intended working coordinates:
...
* Construct an exact parametric description for SunPlayer’s final surface where supported.
* Do not assume the protocol’s predefined Windows-scRGB description supplies the reference-white and target-volume facts SunPlayer needs.
...
3. **Peak detection can change the effective source range.**
   That may be desirable for bad or missing metadata, but its state must reset on seek/open/generation change and must not redefine SunPlayer’s reference-white anchor. libplacebo requires successive frames from one source to share peak-detection state and exposes an explicit reset. ([GitHub][14])

4. **Pin rendering policy.**
   libplacebo 7’s defaults are spline tone mapping and perceptual gamut mapping. SunPlayer should set these explicitly so upgrades do not silently alter output. ([GitHub][14])
...
This is a concrete libplacebo limitation for SunPlayer’s required contract. The right response is an upstream API discussion or patch, not a SunPlayer-authored HLG/tone-mapping shader.
...
3. **Create the production importer seam**
    ...
    * No pixel copy merely to obtain a SunPlayer-owned structure.
...
* Passing metadata through to a display while simultaneously tone-mapping it in SunPlayer.
...
## 3. SunPlayer’s working color space
...
After composition, SunPlayer must tell the operating system what its final pixels mean.
...
SunPlayer should explicitly model these modes rather than vaguely having an optional `iccProfile`.
...
So SunPlayer’s Windows HDR path should be:
...
SunPlayer therefore has a real architectural choice:
...
SunPlayer should therefore not normally take the `NSScreen` display ICC profile and apply it itself. Instead:
...
For SunPlayer, the preferred path should usually be parametric because HDR needs explicit concepts such as:
...
If the compositor offers only an ICC preferred description, SunPlayer may potentially render to that ICC profile using libplacebo and assign the identical description to the surface. But that needs a dedicated experiment: ICC-based preferred descriptions can be asynchronous, may be unsupported, and are not guaranteed to expose parametric information. ([Wayland][icc-3])

Most importantly, SunPlayer must not apply the **physical output profile** and then also ask the compositor to convert the surface to that output. That would again risk double color management.
...
> **libplacebo owns content color processing and perceptual gamut adaptation; the operating system normally owns physical display calibration; SunPlayer applies a display ICC itself only on explicitly unmanaged legacy paths.**
