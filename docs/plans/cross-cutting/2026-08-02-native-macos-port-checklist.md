...
| EDR-2 | Ordinary display transitions | In progress | A user-confirmed Apple-M2/macOS-26 multi-display move and return now preserve video, UI color, and EDR mapping. The fix re-runs QRhi Metal `createOrResize()` after an actual `QWindow::screenChanged`, restoring the layer declaration that Qt's Cocoa backing-property propagation replaces while retaining the device, video, Quick, and compositor resources. Live HDR/SDR switching, hotplug, sleep/wake, and a broader display matrix remain manual gates. |
...
| TEST-1 | Shared and macOS tests | Done | Focused Metal, SDR/HDR, VideoToolbox, subtitle, audio, seek, application-smoke, QML-lint, teardown, and pre-submission target-resize checks pass. The post-transition-fix and post-review Debug suite passes 26/26 in 24.55 seconds; every registered application scenario exits itself. |
