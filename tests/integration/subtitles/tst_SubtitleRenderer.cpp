...
bool readTexture(QRhi& rhi, SubtitleRenderer& renderer, QRhiReadbackResult& result) {
    if (!renderer.texture()) {
        return false;
    }
    ...
    updates->readBackTexture(QRhiReadbackDescription(renderer.texture()), &result);
    commandBuffer->resourceUpdate(updates);
    return rhi.endOffscreenFrame() == QRhi::FrameOpSuccess && complete;
}

BytePixel pixel(QRhiReadbackResult const& result, QRhi const& rhi, int x, int y) {
    int const row = rhi.isYUpInFramebuffer() ? result.pixelSize.height() - 1 - y : y;
    qsizetype const offset = (row * result.pixelSize.width() + x) * 4;
    auto const* value = reinterpret_cast<uchar const*>(result.data.constData() + offset);
    return {value[0], value[1], value[2], value[3]};
}

std::size_t visiblePixelCount(QRhiReadbackResult const& result) {
    std::size_t count = 0;
    auto const* bytes = reinterpret_cast<uchar const*>(result.data.constData());
...
QRect visibleBounds(QRhiReadbackResult const& result, QRhi const& rhi) {
    QRect bounds;
...
QByteArray solidRgba(QSize size, uchar red, uchar green, uchar blue) {
    ...
    for (int pixelIndex = 0; pixelIndex < size.width() * size.height(); ++pixelIndex) {
        *output++ = red;
        *output++ = green;
        *output++ = blue;
        *output++ = 255;
...
void SubtitleRendererTest::rendersAssAndRestoresThePausedRoute() {
    ...
    auto events = std::make_shared<std::vector<SubtitleEvent> const>(std::vector<SubtitleEvent>{{
        ...
        .ass = QByteArrayLiteral("0,0,Default,,0,0,0,,{\\t(0,5000,\\fscx200)}ABCD"),
    }});
    ...
    QSize const targetSize{320, 180};
    QRect const videoRect(QPoint{}, targetSize);
    ...
    QRect const assBounds = visibleBounds(visible, rhi);
    ...
    QRect const animatedBounds = visibleBounds(animated, rhi);
...
void SubtitleRendererTest::replacesAndClearsBitmapCompositions() {
    ...
    auto events = std::make_shared<std::vector<SubtitleEvent> const>(std::vector<SubtitleEvent>{
        {
            ...
            .bitmap = bitmap({2, 3}, {4, 2}, solidRgba({4, 2}, 255, 0, 0)),
        },
        {
            .playbackGeneration = 2,
            .startMicroseconds = 1'000'000,
            .type = SubtitlePayloadType::Bitmap,
            .bitmap = bitmap({20, 10}, {3, 3}, solidRgba({3, 3}, 0, 0, 255)),
        },
        {
            .playbackGeneration = 2,
    ...
    BytePixel const firstPixel = pixel(first, rhi, 13, 9);
    QVERIFY(firstPixel.r > 240 && firstPixel.g < 10 && firstPixel.b < 10 && firstPixel.a == 255);
    ...
    BytePixel const secondPixel = pixel(second, rhi, 40, 21);
    QVERIFY(secondPixel.r < 10 && secondPixel.g < 10 && secondPixel.b > 240 && secondPixel.a == 255);
...
void SubtitleRendererTest::failureIsLatchedUntilNextGeneration() {
    ...
    auto const snapshotFor = [](std::uint64_t generation) {
            ...
            .state =
                {
                    ...
                    .events = std::make_shared<std::vector<SubtitleEvent> const>(std::vector<SubtitleEvent>{{
                        .playbackGeneration = generation,
                        .startMicroseconds = 0,
                        .type = SubtitlePayloadType::Bitmap,
                        .bitmap = bitmap({2, 3}, {4, 2}, solidRgba({4, 2}, 255, 0, 0)),
    ...
    QVERIFY(renderer.prepare(failed, QRect(QPoint{}, QSize(32, 18)), QSize(32, 18), true));
    QRhiReadbackResult visible;
    QVERIFY(readTexture(rhi, renderer, visible));
    QVERIFY(visiblePixelCount(visible) > 0);

    QVERIFY(!renderer.prepare(failed, QRect(0, 0, 1, 1), QSize(16'385, 1), true));
    ...
    QVERIFY(renderer.prepare(recovered, QRect(QPoint{}, QSize(32, 18)), QSize(32, 18), true));
    QVERIFY(renderer.error().isEmpty());
#endif
}
